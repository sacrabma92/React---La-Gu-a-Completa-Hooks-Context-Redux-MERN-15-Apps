...
export default function ActivityList({ activities, dispatch }: ActivitieListProps) {
        ...
        activities.map(activity => (
          <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between">
            <div className="space-y-2 relative">
              <p className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold ${activity.category === 1 ? 'bg-lime-500' : 'bg-blue-500'}`}>{categoryName(+activity.category)}</p>
