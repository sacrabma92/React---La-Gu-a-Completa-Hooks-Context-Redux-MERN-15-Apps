...
const Paciente = ({ paciente, setPaciente, eliminarPaciente }) => {
  ...
  return (
    <div className="mx-5 my-10 bg-white shadow-md px-5 py-10 rounded-xl">
      ...
      <p className="font-bold mb-3 text-gray-700 uppercase">Salida:{' '}<span className="font-normal normal-case">{fecha}</span></p>
      ...
      <div className="flex justify-between mt-10">
        <button className="py-2 px-10 bg-indigo-600 hover:bg-indigo-700 rounded text-white font-bold uppercase" type="button" onClick={() => setPaciente(paciente)}>Editar</button>

        <button className="py-2 px-10 bg-red-600 hover:bg-red-700 rounded text-white font-bold uppercase" type="button" onClick={ () => eliminarPaciente(id)}>Eliminar</button>
      </div>
